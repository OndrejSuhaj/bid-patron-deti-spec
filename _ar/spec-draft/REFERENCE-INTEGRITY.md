...
> Checks declared reference fields (`references` / `affects` / `trigger`) + inline doc_id mentions across the 11 present canonical layers (EN, UC, FN, ES, MSG, BR, ARCH, API, JOB, ACL, QUERY). UX/CS layers (CS/IA/WIRE/COMP/COPY) are self-validated at authoring/publish time, not tracked here → references to them = deferred.
